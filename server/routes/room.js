...
router.get('/orders', authorize, (req,res) => {
    Pool.query(`SELECT * FROM ORDERS WHERE USERID=? ${req.query.active=="true" ? 'AND CANCELLED IS NULL AND EXPIRYDATE>=CURDATE()' : ''} ORDER BY EXPIRYDATE DESC`, [res.user.userId])
    .then(result => {
